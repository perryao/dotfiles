...
n1=`/root/go/bin/vim-startuptime -vimpath nvim | grep "^Total Average:" | cut -d ' ' -f3`
# 3. test startuptime with all plugins
n2=`/root/go/bin/vim-startuptime -vimpath nvim -- -u init.vim | grep "^Total Average:" | cut -d ' ' -f3`
